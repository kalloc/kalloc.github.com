...
def mamba_photo_parser(self):
    ...
    try:
        os.mkdir("mamba_man/"+user_id,0777)
        path="mamba_man/"+user_id+"/"
    except:
        path="mamba_man/"+user_id+"/"
...
if  fp != 0:
    ...
    for row in reader:
	if os.path.exists("mamba_man/"+row[0]):
	    mambauser[row[1]]=1
...
while True:
	...
	mambaquery.append("http://mamba.ru/search.phtml?offset="+str(offset)+"&s_l=M&s_i=N&s_f=18&s_t=25&s_c=3159_4312_4400&s_m=0&s_p=checked&s_w=checked&s_ty=All&s_vf=&s_vw=&s_vip=&s_v=&search_mode=Base&gid=164305355")
    getit()
